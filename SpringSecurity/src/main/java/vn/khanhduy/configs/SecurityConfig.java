...
public class SecurityConfig {
	...
	public UserDetailsService userDetailsService(PasswordEncoder encoder) {
		UserDetails admin = User.withUsername("duy").password(encoder.encode("12345")).roles("ADMIN").build();
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		return http.csrf(csrf -> csrf.disable())
				.authorizeHttpRequests(
						auth -> auth.requestMatchers("/").permitAll().requestMatchers("/customer/**").hasRole("ADMIN")
	 ...
	 * throws Exception { return http.csrf(csrf ->
	 * csrf.disable()).authorizeHttpRequests( auth ->
	 * auth.requestMatchers("/").permitAll().requestMatchers("/customer/**").
